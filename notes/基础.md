...
React.createElement会预先执行检查来检测错误，实际上编写了这样一个对象。这些对象就是React元素，被读取后构建DOM并保持更新
...
ReactDOM管理单一的一个root节点`<div id="root"></div>`，如果吧React放进一个已有应用则可以包含多个根结点。可以吧想要渲染的元素传入`ReactDOM.render()`
